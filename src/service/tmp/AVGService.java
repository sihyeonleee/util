...
public class AVGService extends Service {
	...
	Map<String, JTextArea> area = new HashMap<>();
	Map<String, DBConnection> mainDb = new HashMap<>();
	
	DBConnection local = new DBConnection("local", "jdbc:mysql://localhost/keris",
			DBConnection.MYSQL, "root", "root");
	...
	public AVGService() {
		...
		btn3.setName("사용자 접속수");
		btn3.setEvtName("gradeJoin");
		btn3.setType(CompObj.TYPE_BUTTON);
		btn3.setEventType(CompObj.EVENT_ACTION);
		btn3.setGridSize(1, 1);
		btn3.setGridPosition(3, 7);
		...
		btn4.setName("동적 쿼리");
		btn4.setEvtName("dynamicQuery");
		btn4.setType(CompObj.TYPE_BUTTON);
		btn4.setEventType(CompObj.EVENT_ACTION);
		btn4.setGridSize(2, 1);
		btn4.setGridPosition(4, 0);
		btn4.setGridWeight(1, 1);
		btn4.setArrangeType(GridBagConstraints.BOTH);
		componentObjs.add(btn4);

		CompObj btn5 = new CompObj();
		btn5.setName("대구지역 통계");
		btn5.setEvtName("daeguCreateJoin");
		btn5.setType(CompObj.TYPE_BUTTON);
		btn5.setEventType(CompObj.EVENT_ACTION);
		btn5.setGridSize(2, 1);
		btn5.setGridPosition(0, 7);
		btn5.setGridWeight(1, 1);
		btn5.setArrangeType(GridBagConstraints.BOTH);
		componentObjs.add(btn5);

		CompObj btn6 = new CompObj();
		btn6.setName("메소드 이벤트");
		btn6.setEvtName("dynamicEvent");
		btn6.setType(CompObj.TYPE_BUTTON);
		btn6.setEventType(CompObj.EVENT_ACTION);
		btn6.setGridSize(2, 1);
		btn6.setGridPosition(4, 7);
		btn6.setGridWeight(1, 1);
		btn6.setArrangeType(GridBagConstraints.BOTH);
		componentObjs.add(btn6);
		...
		CompObj area3 = new CompObj();
		area3.setEnabled(false);
		area3.setType(CompObj.TYPE_TEXTAREA);
		area3.setScrollAt(true);
		area3.setGridSize(2, 1);
		area3.setGridWeight(30, 30);
		area3.setGridPosition(0, 4);
		area3.setArrangeType(GridBagConstraints.BOTH);
		componentObjs.add(area3);

		CompObj area4 = new CompObj();
		area4.setEnabled(false);
		area4.setType(CompObj.TYPE_TEXTAREA);
		area4.setScrollAt(true);
		area4.setGridSize(1, 4);
		area4.setGridWeight(30, 30);
		area4.setGridPosition(3, 2);
		area4.setArrangeType(GridBagConstraints.BOTH);
		componentObjs.add(area4);

		CompObj area5 = new CompObj();
		area5.setEnabled(false);
		area5.setType(CompObj.TYPE_TEXTAREA);
		area5.setScrollAt(true);
		area5.setGridSize(1, 4);
		area5.setGridWeight(30, 30);
		area5.setGridPosition(4, 2);
		area5.setArrangeType(GridBagConstraints.BOTH);
		componentObjs.add(area5);
	...
	public void init() {
		...
		area.put("P", textAreas.get(0));
		area.put("M", textAreas.get(1));
		area.put("H", textAreas.get(2));
	...
	public void onEvent(String type, CompObj obj, Object... objects) throws Exception {
		...
		if ("egov".equals(obj.getEvtName())) {
			Thread t = new Thread() {
				@Override
				public void run() {
					Calendar trgetDate = showCalendarPicker();
			...
			t.start();
		} else if ("schul".equals(obj.getEvtName())) {
			Thread t = new Thread() {
				@Override
				public void run() {
					Calendar trgetDate = showCalendarPicker();
			...
			t.start();
		} else if ("gradeJoin".equals(obj.getEvtName())) {
			Thread t = new Thread() {
				@Override
				public void run() {
					Calendar trgetDate = showCalendarPicker();
			...
			t.start();
		} else if ("daeguCreateJoin".equals(obj.getEvtName())) {
			Thread t = new Thread() {
				@Override
				public void run() {
					Calendar trgetDate = showCalendarPicker();
			...
			t.start();
		} else if ("dynamicEvent".equals(obj.getEvtName())) {
			buttons.get(5).setEnabled(false);

			Object objThis = this;

			Class<?> service = Class.forName("service.tmp.AVGService");
			Method[] methodList = service.getMethods();
			List<String> methods = new ArrayList<>();

			for (Method method : methodList) {
				String methodNm = method.getName();
				if (methodNm.startsWith("_")) {
					methods.add(methodNm);
				}
			}
			...
			Thread t = new Thread() {
				...
				public void run() {
					...
					String[] methodNms = methods.toArray(new String[methods.size()]);

					Arrays.sort(methodNms);

					String methodNm = (String) Main.selectPop("메소드명", "", methodNms);
					...
					if (methodNm != null && !methodNm.equals("")) {
						...
						try {
							...
							service = Class.forName("service.tmp.AVGService");
							Method init = service.getMethod(methodNm);
							init.invoke(objThis);
			...
			t.start();
		} else if ("dynamicQuery".equals(obj.getEvtName())) {
	...
	public void egov(Calendar trgetDate) {
		...
		int mode = Main.confirmPop("ONE DAY MODE");
		...
		for (String schulCode : schulCodes) {
			Thread t = new Thread() {
				...
				public void run() {
					...
					area.get(schulCode).setText("");
					area.get(schulCode).setText(" ( " + fmt2.format(dd.getTime()) + " 기준 )");
					System.out.println(" ( " + fmt2.format(dd.getTime()) + " 기준 )");
					egov1(date, schulCode, m0317_2130, m0322_2130, m0405_0100, m0417_0000, mode);
					egov2(date, schulCode, m0317_2130, m0322_2130, m0405_0100, m0417_0000, mode);
					egov3(date, schulCode, m0317_2130, m0322_2130, m0405_0100, m0417_0000, mode);
					egov4(date, schulCode, m0317_2130, m0322_2130, m0405_0100, m0417_0000, mode);
					egovComplCnt++;
	...
	public void egov1(Calendar trgetDate, String schulCode, String m1_0317_2130, String m2_0322_2130, String m0405_0100,
		...
		area.get(schulCode).setCaretPosition(area.get(schulCode).getDocument().getLength());
		...
		if (mode > 0) {
			...
			sCal.set(2020, startMonth, startDay);
		}
		...
		while (true) {
			...
			area.get(schulCode).append("\n" + getShculLevelNm(schulCode) + " 가입수 :" + resultB);
			area.get(schulCode).append(" / " + resultA);
			area.get(schulCode).append("\n" + getShculLevelNm(schulCode) + " 개설수 : " + resultD);
			area.get(schulCode).append(" / " + resultC);
			area.get(schulCode).append("\n");
			area.get(schulCode).setCaretPosition(area.get(schulCode).getDocument().getLength());
	...
	public void egov2(Calendar trgetDate, String schulCode, String m1_0317_2130, String m2_0322_2130, String m0405_0100,
		...
		while (true) {
			...
			area.get(schulCode).append(resultB + " / " + resultA);
			area.get(schulCode).setCaretPosition(area.get(schulCode).getDocument().getLength());
	...
	public void egov4(Calendar trgetDate, String schulCode, String m1_0317_2130, String m2_0322_2130, String m0405_0100,
		...
		while (true) {
			...
			area.get(schulCode).append(getShculLevelNm(schulCode) + "자수 : " + resultA + "\t\t"
					+ getShculLevelNm(schulCode) + "건수 : " + resultB);
			area.get(schulCode).setCaretPosition(area.get(schulCode).getDocument().getLength());
	...
	// 이용자수, 이용건수 -- ( 0617 )
	public void egov4_(Calendar trgetDate, String schulCode, String m1_0317_2130, String m2_0322_2130,
			String m0405_0100, String m0417_0000, int mode) {
		...
		if (mode > 0) {
			sCal.setTime(eCal.getTime());
			sCal.add(Calendar.DATE, ystrDay);
		} else {
			sCal.set(2020, startMonth, startDay);
		}

		sDayStr = s_fmt.format(sCal.getTime());
		eDayStr = e_fmt.format(eCal.getTime());

		// 이용자수
		String userCntQ = "" + "SELECT level, date, COUNT(*) FROM (" + "    SELECT    LCTRE_SN"
				+ "            , REGIST_SN" + "            , DATE_FORMAT(TLCU.REGIST_PNTTM, '%Y%m%d') AS date"
				+ "            , SC.SCHUL_LEVEL_CODE AS level" + "    FROM TB_LEM_CNTNTS_USE TLCU"
				+ "    INNER JOIN TB_LCM_ALCTCR_MANAGE AM ON TLCU.ALCTCR_SN = AM.ALCTCR_SN"
				+ "    INNER JOIN TB_LEM_HMPG_OPER OP ON OP.HMPG_OPER_SN = AM.HMPG_OPER_SN"
				+ "    INNER JOIN TB_SCM_SCHUL SC ON SC.SCHUL_CODE = OP.SCHUL_CODE"
				+ "    INNER JOIN HOST_INFO INFO ON SC.HOST = INFO.HOST AND INFO.LEVEL = SC.SCHUL_LEVEL_CODE"
				+ "    WHERE TLCU.CNTNTS_USE_TY_CODE = '001'" + "    AND AM.ALCTCR_TY = 1"
				+ "    AND DATE_FORMAT(TLCU.REGIST_PNTTM, '%Y%m%d') >= " + sDayStr
				+ "    AND DATE_FORMAT(TLCU.REGIST_PNTTM, '%Y%m%d') < " + eDayStr + "    GROUP BY date, REGIST_SN"
				+ ")a" + "GROUP BY date";

		// 이용건수
		String lctreCntQ = "" + "SELECT level, date, COUNT(*) FROM (" + "    SELECT    LCTRE_SN"
				+ "            , REGIST_SN" + "            , DATE_FORMAT(TLCU.REGIST_PNTTM, '%Y%m%d') AS date"
				+ "            , SC.SCHUL_LEVEL_CODE AS level" + "    FROM TB_LEM_CNTNTS_USE TLCU"
				+ "    INNER JOIN TB_LCM_ALCTCR_MANAGE AM ON TLCU.ALCTCR_SN = AM.ALCTCR_SN"
				+ "    INNER JOIN TB_LEM_HMPG_OPER OP ON OP.HMPG_OPER_SN = AM.HMPG_OPER_SN"
				...
				+ "    WHERE TLCU.CNTNTS_USE_TY_CODE = '001'" + "    AND AM.ALCTCR_TY = 1"
				+ "    AND DATE_FORMAT(TLCU.REGIST_PNTTM, '%Y%m%d') >= " + sDayStr
				+ "    AND DATE_FORMAT(TLCU.REGIST_PNTTM, '%Y%m%d') < " + eDayStr
				+ "    GROUP BY date, LCTRE_SN, REGIST_SN" + ")a" + "GROUP BY date";
	...
	public void schul(Calendar trgetDate) {
		...
		// 학교수 초중
		String schulQuery1_2 = "" 
				+ "select sub.CITY_NM As 시도" 
				+ "  , CASE WHEN sub.SCHUL_LEVEL_CODE = 'P' THEN '초등학교'"
				+ "    WHEN sub.SCHUL_LEVEL_CODE = 'M' THEN '중학교'" 
				+ "    WHEN sub.SCHUL_LEVEL_CODE = 'H' THEN '고등학교'"
				+ "    WHEN sub.SCHUL_LEVEL_CODE = 'A' THEN '대안학교'" 
				+ "    ELSE '기타학교'" 
				+ "  END AS 학제"
				+ "  , count(*) As 학교수" 
				+ " FROM (" 
				+ "  SELECT TSS.CITY_NM" 
				+ "    , TSS.SCHUL_LEVEL_CODE"
				+ "    , TSS.SCHUL_CODE" 
				+ "  FROM TB_LEM_HMPG_OPER TLHO"
				...
				+ "  WHERE TLHO.HMPG_SE_CODE = '005'" 
				+ "  AND TLHO.ESTBL_STTUS_CODE = '005'"
				+ "  AND (TSHR.CLS_STTUS_CODE IS NULL OR TSHR.CLS_STTUS_CODE <> '005')"
				+ "  AND DATE_FORMAT(TLHO.FRST_REGIST_PNTTM ,'%Y%m%d%H%i') <= '" + dateString + "' "
				+ "  AND TSS.SCHUL_LEVEL_CODE = 'H'" 
				+ "  GROUP BY TSS.CITY_NM, TSS.SCHUL_LEVEL_CODE, TSS.SCHUL_CODE"
				+ " ) sub" 
				+ " GROUP BY sub.CITY_NM, sub.SCHUL_LEVEL_CODE";
		// 학교수 고등
		String schulQuery1_1 = "" 
				+ "select sub.CITY_NM As 시도" 
				+ "  , CASE WHEN sub.SCHUL_LEVEL_CODE = 'P' THEN '초등학교'"
				+ "    WHEN sub.SCHUL_LEVEL_CODE = 'M' THEN '중학교'" 
				+ "    WHEN sub.SCHUL_LEVEL_CODE = 'H' THEN '고등학교'"
		...
		// 학급수 초중
		String schulQuery2_2 = "" 
				+ " SELECT TSS.CITY_NM As 시도"
				+ "   , CASE WHEN TSS.SCHUL_LEVEL_CODE = 'P' THEN '초등학교'"
				+ "     WHEN TSS.SCHUL_LEVEL_CODE = 'M' THEN '중학교'" 
				+ "     WHEN TSS.SCHUL_LEVEL_CODE = 'H' THEN '고등학교'"
				+ "     WHEN TSS.SCHUL_LEVEL_CODE = 'A' THEN '대안학교'" 
				+ "     ELSE '기타학교'" 
				+ "   END AS 학제"
				+ "   , COUNT(*) as 학급수" 
				+ " FROM TB_LEM_HMPG_OPER TLHO"
				...
				+ " AND DATE_FORMAT(TLHO.FRST_REGIST_PNTTM ,'%Y%m%d%H%i') <= '" + dateString + "' "
				+ " AND TSS.SCHUL_LEVEL_CODE = 'H'" 
				+ " GROUP BY TSS.CITY_NM, TSS.SCHUL_LEVEL_CODE";
		// 학급수 고등
		String schulQuery2_1 = ""
				+ " SELECT TSS.CITY_NM As 시도"
				+ "   , CASE WHEN TSS.SCHUL_LEVEL_CODE = 'P' THEN '초등학교'"
				+ "     WHEN TSS.SCHUL_LEVEL_CODE = 'M' THEN '중학교'" 
				+ "     WHEN TSS.SCHUL_LEVEL_CODE = 'H' THEN '고등학교'"
		...
		textAreas.get(3).append("( " + pickDate + " 기준 )");

		textAreas.get(3).append("\n #### 학교수 #### ");
		textAreas.get(3).setCaretPosition(textAreas.get(3).getDocument().getLength());
		...
		Map<String, Map<String, Long>> studyCnt = new HashMap<>();
		
		realTimeThreadCnt = 0;
		
		Map<String, Object> param = new HashMap<>();
		SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd");
		param.put("frstPnttm", fmt.format(trgetDate.getTime()));
		...
		for(DBConnection db : allDbs){
			Thread t = new Thread(new Runnable() {
				...
				public void run() {
					...
					// 화상강의 학교 수
					List<Map<String, Object>> schulResult = sql.selectList("avgservice.selectRealTimeSchoolCnt", param);
					...
					// 화상강의 학급 수
					List<Map<String, Object>> classResult = sql.selectList("avgservice.selectRealTimeClassCnt", param);
					...
					// 화상강의 강좌 수
					List<Map<String, Object>> alctcrResult = sql.selectList("avgservice.selectRealTimeGenAlctcrCnt", param);
					...
					// 화상강의 강의 수
					List<Map<String, Object>> lctreResult = sql.selectList("avgservice.selectRealTimeGenLctreCnt", param);
					...
					// 화상강의 이용 수
					List<Map<String, Object>> studyResult = sql.selectList("avgservice.selectRealTimeStudyCnt", param);
					
					for(int i=0;i<studyResult.size();i++){
						
						Map<String, Object> data = studyResult.get(i);
						
						String cityNm = (String) data.get("cityNm");
						String level = (String) data.get("schulLevelCode");
						Long cnt = (Long) data.get("cnt");
						
						if(!studyCnt.containsKey(cityNm)){
							studyCnt.put(cityNm, new HashMap<>());
						}
						
						if(!studyCnt.get(cityNm).containsKey(level)){
							studyCnt.get(cityNm).put(level, 0L);
						}
						
						studyCnt.get(cityNm).put(level, studyCnt.get(cityNm).get(level) + cnt);
					...
					if(realTimeThreadCnt >= allDbs.size()){
						...
						for(String cityNm : cityNms){
							if(studyCnt.containsKey(cityNm)){
								if(studyCnt.get(cityNm).containsKey("A")){
									textAreas.get(3).append("\n" + cityNm + "\t" + "대안학교" + "\t" + studyCnt.get(cityNm).get("A"));
									textAreas.get(3).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(studyCnt.get(cityNm).containsKey("E")){
									textAreas.get(3).append("\n" + cityNm + "\t" + "기타학교" + "\t" + studyCnt.get(cityNm).get("E"));
									textAreas.get(3).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(studyCnt.get(cityNm).containsKey("M")){
									textAreas.get(3).append("\n" + cityNm + "\t" + "중학교" + "\t" + studyCnt.get(cityNm).get("M"));
									textAreas.get(3).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(studyCnt.get(cityNm).containsKey("P")){
									textAreas.get(3).append("\n" + cityNm + "\t" + "초등학교" + "\t" + studyCnt.get(cityNm).get("P"));
									textAreas.get(3).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(studyCnt.get(cityNm).containsKey("H")){
									textAreas.get(3).append("\n" + cityNm + "\t" + "고등학교" + "\t" + studyCnt.get(cityNm).get("H"));
									textAreas.get(3).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
							}
						}
	...
	int _realTimeThreadCnt = 0;
	public synchronized void  _realTimeThreadCnt(){
		System.out.println(++_realTimeThreadCnt);
	}
	public void _DAY_RealTimeLctreCnt(){
		
		Calendar trgetDate = showCalendarPicker();
		if (trgetDate == null) return;

		SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd");
		String frstPnttm = fmt.format(trgetDate.getTime());
		trgetDate.add(Calendar.DATE, 1);
		String lastPnttm = fmt.format(trgetDate.getTime());
		
//		String[] selectItem = {frstPnttm + " 10 시 ~ " + frstPnttm + " 18 시", frstPnttm + " 18 시" + lastPnttm + " 10 시"};
//		String method = (String) Main.selectPop("시간선택", "", selectItem);
		
//		if (method == null || method.trim().equals("")) return;
//		else if(method.trim().equals(selectItem[0])){
//			lastPnttm = frstPnttm + " 18";
//			frstPnttm += " 10";
//		}else if(method.trim().equals(selectItem[1])){
//			frstPnttm += " 18";
//			lastPnttm += " 10";
//		}
		...
		Map<String, Map<String, Long>> schulCnt = new HashMap<>();
		Map<String, Map<String, Long>> classCnt = new HashMap<>();
		Map<String, Map<String, Long>> lctreCnt1 = new HashMap<>();
		Map<String, Map<String, Long>> lctreCnt2 = new HashMap<>();
		Map<String, Map<String, Long>> studyCnt = new HashMap<>();
		
		Map<String, Object> param = new HashMap<>();
		param.put("frstPnttm", frstPnttm);
		param.put("lastPnttm", lastPnttm);
		...
		for(DBConnection db : allDbs){
			Thread t = new Thread(new Runnable() {
				...
				public void run() {
					...
					// 화상강의 학교 수
					List<Map<String, Object>> schulResult = sql.selectList("avgservice.selectRealTimeSchoolDayCnt", param);
					...
					// 화상강의 학급 수
					List<Map<String, Object>> classResult = sql.selectList("avgservice.selectRealTimeClassDayCnt", param);
					...
					// 조종례 화상강의 강의 수
					List<Map<String, Object>> lctreResult1 = sql.selectList("avgservice.selectRealTimeGenAlctcrDayCnt", param);
					
					for(int i=0;i<lctreResult1.size();i++){
						Map<String, Object> data = lctreResult1.get(i);
						
						String cityNm = (String) data.get("cityNm");
						String level = (String) data.get("level");
						Long cnt = (Long) data.get("cnt");
						
						if(!lctreCnt1.containsKey(cityNm)){
							lctreCnt1.put(cityNm, new HashMap<>());
						}
						
						if(!lctreCnt1.get(cityNm).containsKey(level)){
							lctreCnt1.get(cityNm).put(level, 0L);
						}
						
						lctreCnt1.get(cityNm).put(level, lctreCnt1.get(cityNm).get(level) + cnt);
						
					}
					
					// 화상강의 강의 수
					List<Map<String, Object>> lctreResult2 = sql.selectList("avgservice.selectRealTimeGenLctreDayCnt", param);
					
					for(int i=0;i<lctreResult2.size();i++){
						Map<String, Object> data = lctreResult2.get(i);
						
						String cityNm = (String) data.get("cityNm");
						String level = (String) data.get("level");
						Long cnt = (Long) data.get("cnt");
						
						if(!lctreCnt2.containsKey(cityNm)){
							lctreCnt2.put(cityNm, new HashMap<>());
						}
						
						if(!lctreCnt2.get(cityNm).containsKey(level)){
							lctreCnt2.get(cityNm).put(level, 0L);
						}
						
						lctreCnt2.get(cityNm).put(level, lctreCnt2.get(cityNm).get(level) + cnt);
						
					}
					
					// 화상강의 이용 수
					List<Map<String, Object>> studyResult = sql.selectList("avgservice.selectRealTimeStudyDayCnt", param);
					...
					if(_realTimeThreadCnt >= allDbs.size()){
						
						// 화상강의 학교 수
						textAreas.get(4).setText("");
						

						textAreas.get(4).append("화상강의 학교수 ");
						for(String cityNm : cityNms){
							if(schulCnt.containsKey(cityNm)){
								if(schulCnt.get(cityNm).containsKey("A")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "대안학교" + "\t" + schulCnt.get(cityNm).get("A"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(schulCnt.get(cityNm).containsKey("E")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "기타학교" + "\t" + schulCnt.get(cityNm).get("E"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
						...
						textAreas.get(4).append("\n\n");
						textAreas.get(4).append("화상강의 학급수 ");
						for(String cityNm : cityNms){
							if(classCnt.containsKey(cityNm)){
								if(classCnt.get(cityNm).containsKey("A")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "대안학교" + "\t" + classCnt.get(cityNm).get("A"));
						...
						textAreas.get(4).append("\n\n");
						textAreas.get(4).append("화상 조종례 개설수 ");
						for(String cityNm : cityNms){
							if(lctreCnt1.containsKey(cityNm)){
								if(lctreCnt1.get(cityNm).containsKey("A")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "대안학교" + "\t" + lctreCnt1.get(cityNm).get("A"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(lctreCnt1.get(cityNm).containsKey("E")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "기타학교" + "\t" + lctreCnt1.get(cityNm).get("E"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(lctreCnt1.get(cityNm).containsKey("M")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "중학교" + "\t" + lctreCnt1.get(cityNm).get("M"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(lctreCnt1.get(cityNm).containsKey("P")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "초등학교" + "\t" + lctreCnt1.get(cityNm).get("P"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(lctreCnt1.get(cityNm).containsKey("H")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "고등학교" + "\t" + lctreCnt1.get(cityNm).get("H"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
							}
						}
						...
						textAreas.get(4).append("\n\n");
						textAreas.get(4).append("화상수업 개설수 ");
						for(String cityNm : cityNms){
							if(lctreCnt2.containsKey(cityNm)){
								if(lctreCnt2.get(cityNm).containsKey("A")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "대안학교" + "\t" + lctreCnt2.get(cityNm).get("A"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(lctreCnt2.get(cityNm).containsKey("E")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "기타학교" + "\t" + lctreCnt2.get(cityNm).get("E"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(lctreCnt2.get(cityNm).containsKey("M")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "중학교" + "\t" + lctreCnt2.get(cityNm).get("M"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(lctreCnt2.get(cityNm).containsKey("P")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "초등학교" + "\t" + lctreCnt2.get(cityNm).get("P"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
								if(lctreCnt2.get(cityNm).containsKey("H")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "고등학교" + "\t" + lctreCnt2.get(cityNm).get("H"));
									textAreas.get(4).setCaretPosition(textAreas.get(3).getDocument().getLength());
								}
							}
						}
						...
						textAreas.get(4).append("\n\n");
						textAreas.get(4).append("화상강의 이용수 ");
						for(String cityNm : cityNms){
							if(studyCnt.containsKey(cityNm)){
								if(studyCnt.get(cityNm).containsKey("A")){
									textAreas.get(4).append("\n" + cityNm + "\t" + "대안학교" + "\t" + studyCnt.get(cityNm).get("A"));
	...
	public void gradeJoinCnt(Calendar trgetDate) {
		...
		textAreas.get(3).append("( " + currentDay + " 기준 )");
	...
	public void daeguCreateJoin(Calendar trgetDate) {
		...
		String q1 = "" 
				+ " SELECT c.SCHUL_NM, COUNT(*) AS cnt" 
				+ " FROM TB_LEM_HMPG_OPER a"
				...
				+ " AND c.SCHUL_LEVEL_CODE = 'H'" 
				+ " GROUP BY c.SCHUL_CODE";
		...
		String q3 = "" 
				+ " SELECT SCHUL_NM, COUNT(*) AS cnt FROM (" 
				+ " SELECT c.SCHUL_NM, COUNT(*) AS cnt"
				+ " FROM TB_LEM_HMPG_OPER a" 
				+ " INNER JOIN TB_SUM_HMPG_REQST b ON a.HMPG_REQST_SN = b.HMPG_REQST_SN"
				+ " INNER JOIN TB_SCM_SCHUL c ON a.SCHUL_CODE = c.SCHUL_CODE"
				...
				+ " AND DATE_FORMAT(a.FRST_REGIST_PNTTM ,'%Y%m%d%H%i') <= " + dateString + " AND c.CITY_NM = '대구'"
				+ " AND c.SCHUL_LEVEL_CODE = 'H'" 
				+ " GROUP BY c.SCHUL_CODE, d.USER_SN" 
				+ " ) a"
				+ " GROUP BY a.SCHUL_NM";
				...
				public void run() {
					...
					if (lctreGetAndUpCnt >= allDbs.size()) {
						...
						while (true) {
							...
							textAreas.get(4).append(date + " ::: 자체제작 " + makeD + " ::: 기본제공 :" + getD + "\n");
						...
						textAreas.get(4).setCaretPosition(textAreas.get(4).getDocument().getLength());

					} else {
						textAreas.get(4).append("남은 DB : " + (allDbs.size() - lctreGetAndUpCnt) + "\n");
						textAreas.get(4).setCaretPosition(textAreas.get(4).getDocument().getLength());
					}
	...
	public void _DAY_studentJoin() {
		...
		String q = "" + " SELECT  day" + "       ,count(IF(SCHUL_LEVEL_CODE = 'P' and a.GRADE = 1, cnt, null)) As '초1' "
				...
				+ "   AND e.HOST = ?" + "   AND a.FRST_REGIST_PNTTM >= '2020-12-31'"
				+ "   GROUP BY DATE_FORMAT(a.FRST_REGIST_PNTTM, '%Y%m%d'), e.SCHUL_LEVEL_CODE, d.GRADE, b.USER_SN"
				+ " ) a" + " GROUP BY day";

		allDbsToExcelQuery("학생 접속수", q, 2, buttons.get(5));

	}

	// 온클 이솦강의 이용수 (TB_TSM_CNTNTS_TY_USE_STTUS)
	int _esofCntntsUseThreadCnt = 0;
	public synchronized void  _esofCntntsUseThreadCnt(){
		System.out.println(++_esofCntntsUseThreadCnt);
		textAreas.get(4).setText("_DAY_EsofCntntsUseCnt : " + _esofCntntsUseThreadCnt + "%");
	}
	
	public synchronized void  _esofCntntsUseThreadSum(Map<String, Map<String, Map<String, Integer>>> useCnt, String date, String codeNm, String key, int value){
		useCnt.get(date).get(codeNm).put(key, useCnt.get(date).get(codeNm).get(key) + value);
	}
	
	public synchronized void  _esofCntntsUseThreadSum(Map<String, Map<String, Integer>> useCnt, String date, String key, int value){
		useCnt.get(date).put(key, useCnt.get(date).get(key) + value);
	}
	
	public synchronized void  _esofCntntsUseThreadSum_(Map<String, Map<String, Integer>> useCnt, String device, String date, int sum){
		useCnt.get(device).put(date, useCnt.get(device).get(date) + sum);
	}
	
	public void _DAY_EsofCntntsUseCnt_old(){
		
		// selectEsofCntntsUseCnt
		
		List<DateObj> trgetDates = showCalendarPickers();
		if (trgetDates == null || trgetDates.size() < 2) return;

		SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd");
		Calendar sDate = trgetDates.get(0).getCal();
		String frstPnttm = fmt.format(sDate.getTime());
		String lastPnttm = fmt.format(trgetDates.get(1).getCal().getTime());
		
		Map<String, Map<String, Integer>> useCnt = new HashMap<>();
		
		Map<String, Object> param = new HashMap<>();
		param.put("strDate", frstPnttm);
		param.put("endDate", lastPnttm);
		...
		for(DBConnection db : allDbs){
			Thread t = new Thread(new Runnable() {
				...
				public void run() {
					...
					// 화상강의 학교 수
					List<Map<String, Object>> useResult = sql.selectList("avgservice.selectEsofCntntsUseCnt", param);
					
					for(int i=0;i<useResult.size();i++){
						Map<String, Object> data = useResult.get(i);
						
						String device = (String) data.get("device");
						String date = (String) data.get("date");
						int sum = Integer.parseInt(((BigDecimal) data.get("sum")).toString());
						
						if(!useCnt.containsKey(device)){
							useCnt.put(device, new HashMap<>());
						}
						
						if(!useCnt.get(device).containsKey(date)){
							useCnt.get(device).put(date, 0);
						}
						
						_esofCntntsUseThreadSum_(useCnt, device, date, sum);
						
					}
					
					_esofCntntsUseThreadCnt();
					
					if(_esofCntntsUseThreadCnt >= allDbs.size()){
						
						textAreas.get(4).setText("");
						textAreas.get(4).append("\n\n");
						textAreas.get(4).append("ESOF 컨텐츠 이용수\n");
						textAreas.get(4).append("년-월-일\t모바일\tPC");
						
						while(sDate.compareTo(trgetDates.get(1).getCal()) < 1){
							String trgetDate = fmt.format(sDate.getTime());
							textAreas.get(4).append("\n" + trgetDate + "\t" + useCnt.get("MB").get(trgetDate) + "\t" + useCnt.get("PC").get(trgetDate));
							sDate.add(Calendar.DATE, 1);
						}
						
						buttons.get(5).setEnabled(true);
						_esofCntntsUseThreadCnt = 0;
					}
				}
			});
			t.start();
	...
	public void _DAY_EsofCntntsUseCnt(){
		
		// selectEsofCntntsUseCnt
		
		List<DateObj> trgetDates = showCalendarPickers();
		if (trgetDates == null || trgetDates.size() < 1) return;

		SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd");
		String frstPnttm = fmt.format(trgetDates.get(0).getCal().getTime());
		String lastPnttm = "";
		if(trgetDates.size() > 1){
			lastPnttm = fmt.format(trgetDates.get(1).getCal().getTime());
		}else {
			lastPnttm = fmt.format(trgetDates.get(0).getCal().getTime());
		}
		
		Map<String, Map<String, Map<String, Integer>>> useCnt1 = new HashMap<>();
		Map<String, Map<String, Map<String, Integer>>> useCnt2 = new HashMap<>();
		Map<String, Map<String, Integer>> useCnt3 = new HashMap<>();
		...
		for(DBConnection db : allDbs){
			Thread t = new Thread(new Runnable() {
				...
				public void run() {
					...
					// 이솦강의 이용수 강좌별
					List<Map<String, Object>> useResult1 = sql.selectList("avgservice.selectEsofCntntsUseCntAlctcr", param);
					
					for(int i=0;i<useResult1.size();i++){
						Map<String, Object> data = useResult1.get(i);
						
						String date = (String) data.get("date");
						String codeNm = (String) data.get("codeNm");
						String alctcrNm = (String) data.get("alctcrNm");
						int cnt = Integer.parseInt(data.get("cnt").toString());
						
						if(!useCnt1.containsKey(date)){
							useCnt1.put(date, new HashMap<>());
						}
						
						if(!useCnt1.get(date).containsKey(codeNm)){
							useCnt1.get(date).put(codeNm, new HashMap<>());
						}
						
						if(!useCnt1.get(date).get(codeNm).containsKey(alctcrNm)){
							useCnt1.get(date).get(codeNm).put(alctcrNm, 0);
						}
						_esofCntntsUseThreadSum(useCnt1, date, codeNm, alctcrNm, cnt);
					}
					
					
					// 이솦강의 이용수 초중고별
					List<Map<String, Object>> useResult2 = sql.selectList("avgservice.selectEsofCntntsUseCntLevel", param);
					
					for(int i=0;i<useResult2.size();i++){
						Map<String, Object> data = useResult2.get(i);
						...
						if(!useCnt2.containsKey(date)){
							useCnt2.put(date, new HashMap<>());
						}
						
						if(!useCnt2.get(date).containsKey(codeNm)){
							useCnt2.get(date).put(codeNm, new HashMap<>());
						}
						
						if(!useCnt2.get(date).get(codeNm).containsKey(level)){
							useCnt2.get(date).get(codeNm).put(level, 0);
						}
						_esofCntntsUseThreadSum(useCnt2, date, codeNm, level, cnt);
					}
					
					// 이솦강의 이용수 초중고별
					List<Map<String, Object>> useResult3 = sql.selectList("avgservice.selectEsofCntntsUseCntLctre", param);
					
					for(int i=0;i<useResult3.size();i++){
						Map<String, Object> data = useResult3.get(i);
						
						String date = (String) data.get("date");
						String level = (String) data.get("level");
						int cnt = Integer.parseInt(data.get("cnt").toString());
						
						if(!useCnt3.containsKey(date)){
							useCnt3.put(date, new HashMap<>());
						}
						
						if(!useCnt3.get(date).containsKey(level)){
							useCnt3.get(date).put(level, 0);
						}
						_esofCntntsUseThreadSum(useCnt3, date, level, cnt);
					}
					...
					if(_esofCntntsUseThreadCnt >= allDbs.size()){
						...
						textAreas.get(4).append("\n\n");
						textAreas.get(4).append("ESOF 컨텐츠 이용수 (강좌별) \n");
						TreeMap<String, Map<String, Map<String, Integer>>> tMap1 = new TreeMap<>(useCnt1);
						for (Entry<String, Map<String, Map<String, Integer>>> map : tMap1.entrySet()) {
							TreeMap<String, Map<String, Integer>> tMap1_1 = new TreeMap<>(map.getValue());
							for(Entry<String, Map<String, Integer>> m1 : tMap1_1.entrySet()){
								TreeMap<String, Integer> tMap1_2 = new TreeMap<>(m1.getValue());
								for(Entry<String, Integer> m2 : tMap1_2.entrySet()){
									textAreas.get(4).append("\n" + map.getKey() + "\t" + m1.getKey() + "\t" + m2.getKey() + "\t" + m2.getValue());
								}
							}
						}
						
						textAreas.get(4).append("\n\n\n\n");
						textAreas.get(4).append("ESOF 컨텐츠 이용수 (초중고별) \n");
						TreeMap<String, Map<String, Map<String, Integer>>> tMap2 = new TreeMap<>(useCnt2);
						for (Entry<String, Map<String, Map<String, Integer>>> map : tMap2.entrySet()) {
							TreeMap<String, Map<String, Integer>> tMap1_1 = new TreeMap<>(map.getValue());
							for(Entry<String, Map<String, Integer>> m1 : tMap1_1.entrySet()){
								TreeMap<String, Integer> tMap1_2 = new TreeMap<>(m1.getValue());
								for(Entry<String, Integer> m2 : tMap1_2.entrySet()){
						...
						textAreas.get(4).append("\n\n\n\n");
						textAreas.get(4).append("ESOF 컨텐츠 가져오기수 (초중고별) \n");
						TreeMap<String, Map<String, Integer>> tMap3 = new TreeMap<>(useCnt3);
						for (Entry<String, Map<String, Integer>> map : tMap3.entrySet()) {
							TreeMap<String, Integer> tMap3_1 = new TreeMap<>(map.getValue());
							for(Entry<String, Integer> m : tMap3_1.entrySet()){
								textAreas.get(4).append("\n" + map.getKey() + "\t" + m.getKey() + "\t" + m.getValue());
							}
						}
			...
			Thread t = new Thread() {
				...
				public void run() {
					...
					if (dayOfMvpCnt >= allDbs.size()) {
						...
						System.out
								.println("############## 남은 DB " + (allDbs.size() - dayOfMvpCnt) + " ################");
					}
		...
		for (DBConnection db : allDbs) {
			Thread t = new Thread() {
				...
				public void run() {
					...
					textAreas.get(4).append("\n\n");
					textAreas.get(4).append("#################################\n");
					textAreas.get(4).append("현재 남은 작업수 " + (allDbs.size() - findHmpgEmptyDatasThreadCnt) + "\n");
					textAreas.get(4).append("#################################" + "\n");
					textAreas.get(4).append("\n\n");
					...
					if (findHmpgEmptyDatasThreadCnt >= allDbs.size()) {
						...
						textAreas.get(4).append("#################################" + "\n");
						textAreas.get(4).append("#################################" + "\n");
						textAreas.get(4).append("작업 결과 파일 생성중" + "\n");
						textAreas.get(4).append("#################################" + "\n");
						textAreas.get(4).append("#################################" + "\n");
						textAreas.get(4).append("#################################" + "\n");
						w.write();
		...
		for (DBConnection db : allDbs) {
			Thread t = new Thread() {
				...
				public void run() {
					...
					textAreas.get(4).append(db.getName() + " ::: 메뉴 빈 게시판 SELECT \n");
					System.out.println(db.getName() + " ::: 메뉴 빈 게시판 SELECT");
					List<Map<String, Object>> emptyData = (List<Map<String, Object>>) queryM(db, selectQ, db.getName())
							.get("data");
					textAreas.get(4).append(db.getName() + " ::: 빈게시판 갯수 ::: " + emptyData.size() + "\n");
					...
					for (Map<String, Object> data : emptyData) {
						...
						textAreas.get(4).append(db.getName() + " :: " + sheetRowIndex + " / " + emptyData.size()
								+ " ::: " + insertResultStr + "\n");
					...
					if (bbsEmptyMenuCnt >= allDbs.size()) {
						textAreas.get(4).append(" ########### 작업 종료 ########### \n");
						textAreas.get(4).append(" ########## 파일 생성 중 ########## \n");
						...
						textAreas.get(4).append(" ######### 파일 생성 완료 ######### \n");
						textAreas.get(4).append(" ######### " + w.getPath() + " ######### \n");
						System.out.println(" ######### 파일 생성 완료 ######### ");
						System.out.println(" ######### " + w.getPath() + " ######### ");
						textAreas.get(4).setCaretPosition(textAreas.get(4).getDocument().getLength());
					} else {
						textAreas.get(4)
								.append(" ======== 남은 DB수 :: " + (allDbs.size() - bbsEmptyMenuCnt) + " ======== \n");
			...
			Thread t = new Thread() {
				...
				public void run() {
					...
					for (Map<String, Object> data : q1Datas) {
						...
						textAreas.get(4).append(db.getName() + " :: " + rowIndex + " / " + q1Datas.size() + "\n");
						textAreas.get(4).setCaretPosition(textAreas.get(4).getDocument().getLength());
					...
					if (operMberRgstCnt >= allDbs.size()) {
						...
						textAreas.get(4).append(
								" ########## 남은 DB수 : " + (allDbs.size() - operMberRgstCnt) + "개 ############# \n");
						textAreas.get(4).setCaretPosition(textAreas.get(4).getDocument().getLength());
					}
	...
	public void _alctcrEmpty() {
		...
		allDbsToExcelQuery("수강 이력", q, 1, buttons.get(5));
	...
	public void _ESOF_RegistList() {

		List<DateObj> picks = showCalendarPickers();

		SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd");

		if (picks == null || picks.size() < 1) {
			buttons.get(5).setEnabled(true);
			return;
		}
		...
		sDt = picks.get(0).getCal();

		if (picks.size() > 1) {
			eDt = picks.get(1).getCal();
		} else {
			eDt.setTime(sDt.getTime());
		}
		String eDateStr = fmt.format(eDt.getTime());
		eDt.add(Calendar.DATE, 1);

		String sDate = fmt.format(sDt.getTime());
		String eDate = fmt.format(eDt.getTime());

		Map<String, Object> param = new HashMap<>();
		param.put("sDate", sDate);
		param.put("eDate", eDate);

		SqlSession sql = esof.getSqlSession(true);
		List<Map<String, Object>> cmpResult = sql.selectList("avgservice.selectCmpRegisterList", param);
		List<Map<String, Object>> crtResult = sql.selectList("avgservice.selectCrtRegisterList", param);
		List<Map<String, Object>> mntResult = sql.selectList("avgservice.selectMntRegisterList", param);

		ExcelFileWriter w = new ExcelFileWriter(deskTopPath, "기관_크리에이터_멘토_신청_" + eDateStr, "xlsx");
		...
		for (Map<String, Object> map : cmpResult) {

			w.addData("기관", 0, 0, "신청일");
			w.addData("기관", 0, 1, "기관명");
			w.addData("기관", 0, 2, "신청자");
			w.addData("기관", 0, 3, "처리자");
			w.addData("기관", 0, 4, "URL");
			w.addData("기관", 0, 5, "한줄 슬로건");
			w.addData("기관", 0, 6, "태그");
			w.addData("기관", 0, 7, "홈페이지 소개");
			w.addData("기관", 0, 8, "분야");
			w.addData("기관", 0, 9, "언어");
			w.addData("기관", 0, 10, "서비스 대상");
			w.addData("기관", 0, 11, "용도");
			w.addData("기관", 0, 12, "예상학습자수");
			w.addData("기관", 0, 13, "운영 목적");
			w.addData("기관", 0, 14, "기관 활동계획 및 목표");
			w.addData("기관", 0, 15, "상태");

			w.addData("기관", rowIndex, 0, fmt.format((Date) map.get("신청일")));
			w.addData("기관", rowIndex, 1, map.get("기관명"));
			w.addData("기관", rowIndex, 2, map.get("신청자"));
			w.addData("기관", rowIndex, 3, map.get("처리자"));
			w.addData("기관", rowIndex, 4, map.get("URL"));
			w.addData("기관", rowIndex, 5, map.get("한줄 슬로건"));
			w.addData("기관", rowIndex, 6, map.get("태그"));
			w.addData("기관", rowIndex, 7, map.get("홈페이지 소개"));
			w.addData("기관", rowIndex, 8, map.get("분야"));
			w.addData("기관", rowIndex, 9, map.get("언어"));
			w.addData("기관", rowIndex, 10, map.get("서비스 대상"));
			w.addData("기관", rowIndex, 11, map.get("용도"));
			w.addData("기관", rowIndex, 12, map.get("예상학습자수"));
			w.addData("기관", rowIndex, 13, map.get("운영 목적"));
			w.addData("기관", rowIndex, 14, map.get("기관 활동계획 및 목표"));
			w.addData("기관", rowIndex, 15, map.get("상태"));
			rowIndex++;
		}

		rowIndex = 1;

		for (Map<String, Object> map : crtResult) {

			TreeMap<String, Object> tMap = new TreeMap<String, Object>(map);

			int colIndex = 0;
			for (Map.Entry<String, Object> data : tMap.entrySet()) {

				// ColName Set
				if (rowIndex == 1)
					w.addData("크리에이터", 0, colIndex, data.getKey());

				// ColData Set
				w.addData("크리에이터", rowIndex, colIndex++, data.getValue());
			}
			rowIndex++;
		}
		...
		for (Map<String, Object> map : mntResult) {
			...
			for (Map.Entry<String, Object> data : tMap.entrySet()) {
				...
				// ColName Set
				if (rowIndex == 1)
					w.addData("멘토", 0, colIndex, data.getKey());

				// ColData Set
				w.addData("멘토", rowIndex, colIndex++, data.getValue());
			}
			rowIndex++;
		}
		...
		if (result < 1) {
			TrayIconHandler.displayMessage("FAIL",
					"파일저장 실패 :: 기관_크리에이터_멘토_신청_" + eDateStr + ".xlsx 파일이 열려있거나 로그를 확인하세요.", MessageType.ERROR);
		} else {
			TrayIconHandler.displayMessage("SUCCESS", "기관_크리에이터_멘토_신청_" + eDateStr + ".xlsx 바탕화면 저장 완료",
					MessageType.INFO);
		}

	}

	public void _ESOF_CodingParty1() {
		try {
			...
			String sDate = fmt2.format(sDt.getTime());
			...
			textAreas.get(4).append("코딩파티 시즌1 통계 " + sDate + " ~ " + fmt2.format(eDt.getTime()) + "\n");

			while (true) {

				param.put("Dt", fmt.format(sDt.getTime()));
				textAreas.get(4).append(fmt.format(sDt.getTime()) + "\n");

				textAreas.get(4).append("팽수 \n");
				Map<String, Object> psResult = sql.selectOne("avgservice.selectCodingPartyPs", param);
				textAreas.get(4).append("뚜앙 \n");
				Map<String, Object> daResult = sql.selectOne("avgservice.selectCodingPartyDa", param);
				textAreas.get(4).append("점박웹 \n");
				Map<String, Object> jbwResult = sql.selectOne("avgservice.selectCodingPartyJbw", param);
				textAreas.get(4).append("점박설치 \n");
				Map<String, Object> jbiResult = sql.selectOne("avgservice.selectCodingPartyJbi", param);

				// 팽수
				result += psResult.size() > 0 ? psResult.get("pa") + "\t" + psResult.get("ps") + "\t"
						+ psResult.get("pa") + "\t" + psResult.get("ps") + "\t"
						: 0 + "\t" + 0 + "\t" + 0 + "\t" + 0 + "\t";
				// 뚜앙
				result += daResult.size() > 0 ? daResult.get("d1a") + "\t" + daResult.get("d1s") + "\t"
						: 0 + "\t" + 0 + "\t";
				result += daResult.size() > 0 ? daResult.get("d2a") + "\t" + daResult.get("d2s") + "\t"
						: 0 + "\t" + 0 + "\t";
				result += daResult.size() > 0 ? daResult.get("d3a") + "\t" + daResult.get("d3s") + "\t"
						: 0 + "\t" + 0 + "\t";
				result += daResult.size() > 0 ? daResult.get("d4a") + "\t" + daResult.get("d4s") + "\t"
						: 0 + "\t" + 0 + "\t";

				if (daResult.size() > 0) {
					result += (long) daResult.get("d1a") + (long) daResult.get("d2a") + (long) daResult.get("d3a")
							+ (long) daResult.get("d4a") + "\t";
					result += (long) daResult.get("d1s") + (long) daResult.get("d2s") + (long) daResult.get("d3s")
							+ (long) daResult.get("d4s") + "\t";
				} else {
					result += 0 + "\t" + 0 + "\t";
				}

				// 점박 웹
				result += jbwResult.size() > 0 ? jbwResult.get("jwa") + "\t" + jbwResult.get("jws") + "\t"
						+ jbwResult.get("jwa") + "\t" + jbwResult.get("jws") + "\t"
						: 0 + "\t" + 0 + "\t" + 0 + "\t" + 0 + "\t";

				// 점박 설치
				result += jbiResult.size() > 0 ? jbiResult.get("d1a") + "\t" + jbiResult.get("d1s") + "\t"
						: 0 + "\t" + 0 + "\t";
				result += jbiResult.size() > 0 ? jbiResult.get("d2a") + "\t" + jbiResult.get("d2s") + "\t"
						: 0 + "\t" + 0 + "\t";
				result += jbiResult.size() > 0 ? jbiResult.get("d3a") + "\t" + jbiResult.get("d3s") + "\t"
						: 0 + "\t" + 0 + "\t";

				if (daResult.size() > 0) {
					result += (long) jbiResult.get("d1a") + (long) jbiResult.get("d2a") + (long) jbiResult.get("d3a")
							+ "\t";
					result += (long) jbiResult.get("d1s") + (long) jbiResult.get("d2s") + (long) jbiResult.get("d3s");
				} else {
					result += 0 + "\t" + 0;
				}

				if (fmt.format(sDt.getTime()).compareTo(fmt.format(eDt.getTime())) == 0) {
					break;
				} else {
			...
			buttons.get(5).setEnabled(true);
			TrayIconHandler.displayMessage("클립보드 복사 완료", "온라인 코딩파티 시즌1 " + sDate + " ~ " + fmt2.format(eDt.getTime()),
					MessageType.INFO);
		} catch (Exception err) {
			err.printStackTrace();
			TrayIconHandler.displayMessage("ERROR", err.getMessage(), MessageType.ERROR);
		}
	}

	public void _ESOF_CodingParty1_JB() {
		try {
			...
			List<List<Map<String, Object>>> joinDataList = new ArrayList<>();
			List<List<Map<String, Object>>> cmplDataList = new ArrayList<>();

			textAreas.get(4).append("## 코딩파티 시즌1 점박이 통계 ## \n" + sDate + " ~ " + fmt2.format(eDt.getTime()) + "\n");

			for (int i = 1; i <= 3; i++) {
				param.put("stepNumber", i);

				List<Map<String, Object>> joinResult = sql.selectList("avgservice.selectCodingPartyJbJoin", param);
				List<Map<String, Object>> cmplResult = sql.selectList("avgservice.selectCodingPartyJbCmpl", param);

				joinDataList.add(joinResult);
				cmplDataList.add(cmplResult);
			}

			for (int i = 0; i < joinDataList.get(0).size(); i++) {

				result += (long) joinDataList.get(0).get(i).get("cnt") + "\t\t";
				result += ((BigDecimal) cmplDataList.get(0).get(i).get("cnt")).toString() + "\t";

				result += (long) joinDataList.get(1).get(i).get("cnt") + "\t\t";
				result += ((BigDecimal) cmplDataList.get(1).get(i).get("cnt")).toString() + "\t";

				result += (long) joinDataList.get(2).get(i).get("cnt") + "\t\t";
				result += ((BigDecimal) cmplDataList.get(2).get(i).get("cnt")).toString() + "\n";
			}
			...
			buttons.get(5).setEnabled(true);
			TrayIconHandler.displayMessage("클립보드 복사 완료",
					"온라인 코딩파티 시즌1 점박설치형" + sDate + " ~ " + fmt2.format(eDt.getTime()), MessageType.INFO);
	...
	public void _ESOF_CodingParty2_Join() {
		try {
			...
			textAreas.get(4).append("## 코딩파티 시즌2 참가자수 집계 ## \n" + sDate + " ~ " + fmt2.format(eDt.getTime()) + "\n");

			List<Map<String, Object>> bgJoinResult = sql.selectList("avgservice.selectCodingParty2BgJoin", param);
			List<Map<String, Object>> bgCmplResult = sql.selectList("avgservice.selectCodingParty2BgCmpl", param);
			List<Map<String, Object>> dgJoinResult = sql.selectList("avgservice.selectCodingParty2DgJoin", param);
			List<Map<String, Object>> dgCmplResult = sql.selectList("avgservice.selectCodingParty2DgCmpl", param);

			for (int i = 0; i < bgJoinResult.size(); i++) {

				result += bgJoinResult.get(i).get("S_DATE") + "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t";

				result += ((BigDecimal) bgJoinResult.get(i).get("cnt")).toString() + "\t";
				result += ((BigDecimal) bgCmplResult.get(i).get("cnt")).toString() + "\t\t\t\t\t\t\t";

				result += (long) dgJoinResult.get(i).get("cnt") + "\t";
				result += ((BigDecimal) dgCmplResult.get(i).get("cnt")).toString() + "\n";
			...
			buttons.get(5).setEnabled(true);
			TrayIconHandler.displayMessage("클립보드 복사 완료",
					"코딩파티 시즌2" + sDate + " ~ " + fmt2.format(eDt.getTime()), MessageType.INFO);
	...
	public void _ESOF_CodingNewgame_Join() {
		try {
			...
			textAreas.get(4).append("## NEW 비트고, 두드리고 참가자수 집계 ## \n" + sDate + " ~ " + fmt2.format(eDt.getTime()) + "\n");

			List<Map<String, Object>> bgJoinResult = sql.selectList("avgservice.selectNewGameBgJoin", param);
			List<Map<String, Object>> bgCmplResult = sql.selectList("avgservice.selectNewGameBgCmpl", param);
			List<Map<String, Object>> dgJoinResult = sql.selectList("avgservice.selectNewGameDgJoin", param);
			List<Map<String, Object>> dgCmplResult = sql.selectList("avgservice.selectNewGameDgCmpl", param);
			...
			buttons.get(5).setEnabled(true);
			TrayIconHandler.displayMessage("클립보드 복사 완료",
					"NEW 비트고, 두드리고 참가자수 집계" + sDate + " ~ " + fmt2.format(eDt.getTime()), MessageType.INFO);
	...
	public void _ESOF_CodingParty2_Join2() {
		try {
			...
			List<Map<String, Object>> bgJoinResult = sql.selectList("avgservice.selectCodingParty2BgJoin2", param);
			List<Map<String, Object>> dgJoinResult = sql.selectList("avgservice.selectCodingParty2DgJoin2", param);
			...
			for (int i = 0; i < bgJoinResult.size(); i++) {
				...
				result += (BigDecimal) dgJoinResult.get(i).get("cnt") + "\t\n";
	...
	public void _ESOF_CodingNewgame_Join2() {
		try {
			...
			List<Map<String, Object>> bgJoinResult = sql.selectList("avgservice.selectNewGameBgJoin2", param);
			List<Map<String, Object>> dgJoinResult = sql.selectList("avgservice.selectNewGameDgJoin2", param);
	...
	public void _ESOF_CodingProject_Join(){
		SqlSession sql = esof.getSqlSession(true);
		
		textAreas.get(4).setText("");
		textAreas.get(4).append("## 코딩 프로젝트 참가자수 집계 ## \n");
		List<Map<String, Object>> resultList = sql.selectList("avgservice.selectCdngPrjct398JoinCnt");
		
		for(Map<String, Object> map : resultList){
			textAreas.get(4).append(map.get("date") + "\t" + map.get("cnt") + "\n");
		}
	...
	public void _ESOF_CodingProject_Join496(){
		SqlSession sql = esof.getSqlSession(true);
		
		textAreas.get(4).setText("");
		textAreas.get(4).append("## 코딩 프로젝트 참가자수 집계 ## \n");
		List<Map<String, Object>> resultList = sql.selectList("avgservice.selectCdngPrjct496JoinCnt");
	...
	public void _ESOF_CdngEduEventCnt(){
		...
		String frstPnttm = fmt.format(trgetDates.get(0).getCal().getTime());
		String frstPnttm2 = fmt2.format(trgetDates.get(0).getCal().getTime());
		String lastPnttm = "";
		String lastPnttm2 = "";
		if(trgetDates.size() > 1){
			lastPnttm = fmt.format(trgetDates.get(1).getCal().getTime());
			lastPnttm2 = fmt2.format(trgetDates.get(1).getCal().getTime());
		}else {
			lastPnttm = fmt.format(trgetDates.get(0).getCal().getTime());
			lastPnttm2 = fmt2.format(trgetDates.get(0).getCal().getTime());
		}
		...
		SqlSession sql = esof.getSqlSession(true);
		List<Map<String, Object>> alctcrCnt = sql.selectList("avgservice.selectCdngEduEvtAlctcrUse", param);
		
		List<Map<String, Object>> gameCnt = sql.selectList("avgservice.selectCdngEduEvtGameUse", param);
		
		List<Map<String, Object>> joinCnt = sql.selectList("avgservice.selectCdngEduEvtJoinCnt", param);
		
		List<Map<String, Object>> cmplViewCnt = sql.selectList("avgservice.selectCdngEduEvtCertCnt", param);
		
		textAreas.get(4).setText("");
		
		ExcelFileWriter w = new ExcelFileWriter(deskTopPath, "코딩교육통계_" + frstPnttm2 + "-" + lastPnttm2, "xlsx");
		
		int col=0;
		String sheet1 = "참가자수";
		String sheet2 = "접속자수, 이수자수";
		String sheet3 = "콘텐츠활용건수-강좌";
		String sheet4 = "콘텐츠활용건수-게임";
		
		// init
		w.addData(sheet1, 0, 0, "");
		w.addData(sheet2, 0, 0, "");
		w.addData(sheet3, 0, 0, "");
		w.addData(sheet4, 0, 0, "");
		
		// title
		w.addData(sheet1, 0, 0, "#아래 데이터는 응모하기 수(응모신청자수)");
		w.addData(sheet1, 1, 0, "년월일");
		w.addData(sheet1, 1, 1, "카운트");
		col=0;
		for(int i=0;i<joinCnt.size();i++){
			Map<String, Object> map = joinCnt.get(i);
			w.addData(sheet1, i+2, (col++)%2, map.get("day"));
			w.addData(sheet1, i+2, (col++)%2, map.get("cnt"));
		}
		
		textAreas.get(4).append(sheet1 + "\t O\n");
		
		// title
		w.addData(sheet2, 0, 0, "#아래의 데이터는 누적데이터로 계산이 필요함");
		w.addData(sheet2, 1, 0, "년월일");
		w.addData(sheet2, 1, 1, "조회수");
		w.addData(sheet2, 1, 2, "전체과정");
		w.addData(sheet2, 1, 3, "과정1");
		w.addData(sheet2, 1, 4, "과정2");
		w.addData(sheet2, 1, 5, "과정3");
		col=0;
		for(int i=0;i<cmplViewCnt.size();i++){
			Map<String, Object> map = cmplViewCnt.get(i);
			w.addData(sheet2, i+2, (col++)%6, map.get("day"));
			w.addData(sheet2, i+2, (col++)%6, map.get("view"));
			w.addData(sheet2, i+2, (col++)%6, map.get("cmpl"));
			w.addData(sheet2, i+2, (col++)%6, map.get("cmpl1"));
			w.addData(sheet2, i+2, (col++)%6, map.get("cmpl2"));
			w.addData(sheet2, i+2, (col++)%6, map.get("cmpl3"));
		}
		textAreas.get(4).append(sheet2 + "\t O\n");
		
		// title
		w.addData(sheet3, 0, 0, "#강좌_강의_컨텐츠별 조회수");
		w.addData(sheet3, 1, 0, "년월일");
		w.addData(sheet3, 1, 1, "강좌명");
		w.addData(sheet3, 1, 2, "강의명");
		w.addData(sheet3, 1, 3, "카운트");
		col=0;
		for(int i=0;i<alctcrCnt.size();i++){
			Map<String, Object> map = alctcrCnt.get(i);
			w.addData(sheet3, i+2, (col++)%4, map.get("day"));
			w.addData(sheet3, i+2, (col++)%4, map.get("alctcrNm"));
			w.addData(sheet3, i+2, (col++)%4, map.get("lctreNm"));
			w.addData(sheet3, i+2, (col++)%4, map.get("cnt"));
		}
		textAreas.get(4).append(sheet3 + "\t O\n");
		
		// title
		w.addData(sheet4, 0, 0, "#펭수,뚜앙 조회수");
		
		w.addData(sheet4, 1, 0, "뚜앙");
		w.addData(sheet4, 1, 5, "펭수");
		
		w.addData(sheet4, 2, 0, "년월일");
		w.addData(sheet4, 2, 1, "스테이지");
		w.addData(sheet4, 2, 2, "미션");
		w.addData(sheet4, 2, 3, "카운트");
		
		w.addData(sheet4, 2, 5, "년월일");
		w.addData(sheet4, 2, 6, "스테이지");
		w.addData(sheet4, 2, 7, "미션");
		w.addData(sheet4, 2, 8, "카운트");
//		w.addData(sheet4, 1, 1, "뚜앙_1");
//		w.addData(sheet4, 1, 2, "뚜앙_1");
//		w.addData(sheet4, 1, 3, "뚜앙_1");
//		w.addData(sheet4, 1, 4, "뚜앙_1");
//		w.addData(sheet4, 1, 5, "펭수_1");
//		w.addData(sheet4, 1, 6, "펭수_2");
//		w.addData(sheet4, 1, 7, "펭수_3");
//		w.addData(sheet4, 1, 8, "펭수_4");
//		w.addData(sheet4, 1, 9, "펭수_5");
//		w.addData(sheet4, 1, 10, "펭수_6");
		col=0;
		int dduLow = 0;
		int penLow = 0;
		for(int i=0;i<gameCnt.size();i++){
			Map<String, Object> map = gameCnt.get(i);
			if("ESOF01".equals(map.get("lrnCours"))){
				w.addData(sheet4, dduLow+3, (col++)%4, map.get("day"));
				w.addData(sheet4, dduLow+3, (col++)%4, map.get("lrnStage"));
				w.addData(sheet4, dduLow+3, (col++)%4, map.get("lrnMisn"));
				w.addData(sheet4, dduLow+3, (col++)%4, map.get("cnt"));
				dduLow++;
			}else {
				w.addData(sheet4, penLow+3, ((col++)%4)+5, map.get("day"));
				w.addData(sheet4, penLow+3, ((col++)%4)+5, map.get("lrnStage"));
				w.addData(sheet4, penLow+3, ((col++)%4)+5, map.get("lrnMisn"));
				w.addData(sheet4, penLow+3, ((col++)%4)+5, map.get("cnt"));
				penLow++;
			}
//			w.addData(sheet4, i+2, (col++)%11, map.get("dduJoin1"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("dduJoin2"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("dduJoin3"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("dduJoin4"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("penJoin1"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("penJoin2"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("penJoin3"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("penJoin4"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("penJoin5"));
//			w.addData(sheet4, i+2, (col++)%11, map.get("penJoin6"));
		}
		textAreas.get(4).append(sheet4 + "\t O\n");
		
		w.write();
		textAreas.get(4).append("완료");
	...
	public void tmp() {
		String q1 = "" + "    SELECT DATE_FORMAT(LM.FRST_REGIST_PNTTM , '%Y-%m-%d') As '날짜'"
				+ " , count(if(CNTNTS_TY_CODE = '001', LM.LCTRE_SN, null ) ) As '동영상(업로드)'"
				+ " , count(if(CNTNTS_TY_CODE = '014', LM.LCTRE_SN, null ) ) As '동영상(유튜브)'"
				+ " , count(if(CNTNTS_TY_CODE = '015', LM.LCTRE_SN, null ) ) As '동영상(가져오기)'"
				+ " , count(if(CNTNTS_TY_CODE = '005', LM.LCTRE_SN, null ) ) As '이미지'"
				+ " , count(if(CNTNTS_TY_CODE = '006', LM.LCTRE_SN, null ) ) As '문서'"
				+ " , count(if(CNTNTS_TY_CODE = '007', LM.LCTRE_SN, null ) ) As '퀴즈'"
				+ " , count(if(CNTNTS_TY_CODE = '008', LM.LCTRE_SN, null ) ) As '토론'"
				+ " , count(if(CNTNTS_TY_CODE = '012', LM.LCTRE_SN, null ) ) As '텍스트'" + " FROM TB_LCM_LCTRE_MANAGE LM"
				+ "   INNER JOIN TB_LCM_STEP_MANAGE SM ON LM.STEP_SN = SM.STEP_SN"
				+ "   INNER JOIN TB_LCM_ALCTCR_MANAGE AM ON SM.ALCTCR_SN = AM.ALCTCR_SN"
				+ "   INNER JOIN TB_LEM_HMPG_OPER HO ON AM.HMPG_OPER_SN = HO.HMPG_OPER_SN"
				+ "   INNER JOIN TB_SCM_SCHUL SC ON HO.SCHUL_CODE = SC.SCHUL_CODE" + " WHERE HO.HMPG_SE_CODE = '005'"
				+ " AND HO.OPER_AT = 1" + " AND (AM.CLS_AT <> 1 OR AM.CLS_AT IS NULL)"
				+ " AND LM.FRST_REGIST_PNTTM > '2020-03-01'" + " AND LM.FRST_REGIST_PNTTM < '2020-04-22'"
				+ " AND SC.HOST = ?" + " GROUP BY DATE_FORMAT(LM.FRST_REGIST_PNTTM , '%Y-%m-%d')";
		String q1_1 = "" + "    SELECT DATE_FORMAT(LM.FRST_REGIST_PNTTM , '%Y-%m-%d') As '날짜'"
				+ " , count(if(CNTNTS_TY_CODE = '015', LM.LCTRE_SN, null ) ) As '동영상(가져오기)'"
				+ " , count(if(CNTNTS_TY_CODE = '001' or CNTNTS_TY_CODE = '014' or CNTNTS_TY_CODE = '005' or CNTNTS_TY_CODE = '006' "
				+ "            or CNTNTS_TY_CODE = '007' or CNTNTS_TY_CODE = '008' or CNTNTS_TY_CODE = '012', LM.LCTRE_SN, null ) ) As '직접 만들기'"
				+ " FROM TB_LCM_LCTRE_MANAGE LM" + "   INNER JOIN TB_LCM_STEP_MANAGE SM ON LM.STEP_SN = SM.STEP_SN"
				+ "   INNER JOIN TB_LCM_ALCTCR_MANAGE AM ON SM.ALCTCR_SN = AM.ALCTCR_SN"
				+ "   INNER JOIN TB_LEM_HMPG_OPER HO ON AM.HMPG_OPER_SN = HO.HMPG_OPER_SN"
				+ "   INNER JOIN TB_SCM_SCHUL SC ON HO.SCHUL_CODE = SC.SCHUL_CODE" + " WHERE HO.HMPG_SE_CODE = '005'"
				+ " AND HO.OPER_AT = 1" + " AND (AM.CLS_AT <> 1 OR AM.CLS_AT IS NULL)"
		...
		String q2 = "" + " SELECT DATE_FORMAT(LM.FRST_REGIST_PNTTM , '%Y-%m-%d') As '날짜'"
				+ " , count(if(CNTNTS_TY_CODE = '001', LM.LCTRE_SN, null ) ) As '동영상(업로드)'"
				+ " , count(if(CNTNTS_TY_CODE = '014', LM.LCTRE_SN, null ) ) As '동영상(유튜브)'"
				+ " , count(if(CNTNTS_TY_CODE = '015', LM.LCTRE_SN, null ) ) As '동영상(가져오기)'"
				+ " , count(if(CNTNTS_TY_CODE = '005', LM.LCTRE_SN, null ) ) As '이미지'"
				...
				+ "   INNER JOIN TB_LEM_HMPG_OPER HO ON AM.HMPG_OPER_SN = HO.HMPG_OPER_SN"
				+ "   INNER JOIN TB_SCM_SCHUL SC ON HO.SCHUL_CODE = SC.SCHUL_CODE" + " WHERE HO.HMPG_SE_CODE = '005'"
				+ " AND HO.OPER_AT = 1" + " AND LM.FRST_REGIST_PNTTM > '2020-03-01'"
				+ " AND LM.FRST_REGIST_PNTTM < '2020-04-22'" + " AND SC.HOST = ?"
				+ " GROUP BY DATE_FORMAT(LM.FRST_REGIST_PNTTM , '%Y-%m-%d')";

		String q3 = "" + " SELECT BBS_ID, a.FRST_REGIST_PNTTM, COUNT(*) AS a  " + " FROM tb_smm_hmpg_menu a"
				+ " INNER JOIN TB_LEM_HMPG_OPER b ON a.HMPG_OPER_SN = b.HMPG_OPER_SN"
				+ " INNER JOIN tb_scm_schul c on b.SCHUL_CODE = c.SCHUL_CODE" + " WHERE BBS_ID IS NOT NULL "
				+ " AND b.OPER_AT = 1" + " and c.HOST = ?" + " GROUP BY BBS_ID " + " HAVING a > 1";

		String q4 = "" + " SELECT COUNT(*)  FROM ( " + " SELECT BBS_ID, a.FRST_REGIST_PNTTM, COUNT(*) AS a  "
				+ " FROM tb_smm_hmpg_menu a" + " INNER JOIN TB_LEM_HMPG_OPER b ON a.HMPG_OPER_SN = b.HMPG_OPER_SN"
				+ " INNER JOIN tb_scm_schul c on b.SCHUL_CODE = c.SCHUL_CODE" + " WHERE BBS_ID IS NOT NULL "
				+ " AND b.OPER_AT = 1" + " and c.HOST = ?" + " GROUP BY BBS_ID " + " HAVING a > 1" + " ) a";

		allDbsToExcelQuery("test", q1_1, 2, buttons.get(5));
	...
	public void dynamicQuery() {
		...
		int result = Main.confirmPop(p);

		String q = area.getText();

		if (result == 1 && !q.trim().equals("") && q != null) {

			String method = (String) Main.selectPop("쿼리결과", "", new String[] { "SUM", "APPEND", "COUNT" });
			...
			if (!method.equals("COUNT")) {
				fileNm = Main.inputPop("파일명 입력");
				if (fileNm == null || fileNm.trim().equals(""))
					fileNm = "temp_excel_file" + UUID.randomUUID();
			}

			// 쿼리 실행
			if (method.equals("COUNT")) {
				allDbsToExcelQuery("", q, 0, buttons.get(3));
			} else if (method.equals("APPEND")) {
				allDbsToExcelQuery(fileNm, q, 1, buttons.get(3));
			} else if (method.equals("SUM")) {
				allDbsToExcelQuery(fileNm, q, 2, buttons.get(3));
			}
	...
	public void allDbsToExcelQuery(String fileName, String Query, int mode, JButton btn) {
			...
			Thread t = new Thread() {
				...
				public void run() {
					...
					if (allDbsToExcelQueryCnt >= allDbs.size()) {
						...
						textAreas.get(4).append(" ########## 남은 DB수 : " + (allDbs.size() - allDbsToExcelQueryCnt)
								+ "개 ############# \n");
						textAreas.get(4).setCaretPosition(textAreas.get(4).getDocument().getLength());
					}
	...
	public Calendar showCalendarPicker() {
		CalendarViewer calendar = new CalendarViewer(CalendarViewer.SELECT_MODE_SINGLE);

		String[] options = { "OK", "Cancel" };
		int result = JOptionPane.showOptionDialog(null, calendar, "Calendar", JOptionPane.YES_NO_OPTION,
				JOptionPane.PLAIN_MESSAGE, null, options, null);

		if (result == JOptionPane.YES_OPTION) {
			return calendar.getNowPickDate();
		}
	...
	public List<DateObj> showCalendarPickers() {
		...
		if (result == JOptionPane.YES_OPTION) {
			System.out.println(calendar.getPickOrderDates().size());
			return calendar.getPickOrderDates();
		}
